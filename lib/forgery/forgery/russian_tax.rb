...
class Forgery::RussianTax < Forgery
  ...
  #bank identification
  def self.bik
    "04#{5.times.map { rand(1..9) }.join}#{rand(50..99)}"
  end

  #bank account format pretty simple
  def self.account_number
    20.times.map { rand(1..9) }.join
  end
  ...
  def self.person_inn
    ...
    inn = 12.times.map { rand(1..9)}.join
